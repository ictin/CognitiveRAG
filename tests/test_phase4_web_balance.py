...
def test_full_memory_web_is_capped_and_non_web_survives():
    ...
    non_web_sources = [s for s in sources if s.get("source_type") != "web"]

    assert decision, trace
    assert decision.get("used") is True, decision
    assert len(web_sources) <= 2, sources
    assert len(non_web_sources) >= 1, sources


def test_documents_only_still_blocks_web_when_forced():
    with TestClient(app) as client:
        services = client.app.state.services
        orig_route, forced_route = _force_route_use_web(services)
        orig_search = services.retriever.web_search.search
